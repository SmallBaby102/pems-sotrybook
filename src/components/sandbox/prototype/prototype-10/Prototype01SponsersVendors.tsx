...
const SponsersVendorsRow : React.FC<{icon:any, header:string, desc:string}> = ({icon, header, desc})=>(
...
const Prototype01SponsersVendorsTabButton : React.FC<{title: string, onClick?: ()=>any, active: boolean, className?: string}> = ({title, onClick, active, className})=>(
    <button onClick={onClick} className={`px-4 py-1 rounded-full text-[#122B46] ${className} ${active && `bg-gray-100 md:bg-transparent md:text-[#F7074F] md:font-medium md:border-b-2 md:border-[#F7074F] md:border-solid md:rounded-none`}`}>{title}</button>
)
